...
class safe_mine_session:
    ...
    On ``__exit__``, flushes ChromaDB's compactor via ``close_palace()``
    *before* restoring the original signal handler — closing the window
    where a Ctrl+C during flush could corrupt the HNSW index.
